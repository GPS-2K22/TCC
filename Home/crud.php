...
<?php
	...
	$con = new mysqli("127.0.0.1:3306", "root", "", "banco");
	...
	if($act == "c"){ //se a acao for para inserir (cadastrar)
		$sql = "insert into produtos (nome, imagem, idade) values ('$nome', '$imagem','$idade')"; //SQL utilizado para consulta
	...
	if($act == "r"){ //se a acao for para recuperar/ler dados
		$sql = "select * from produtos where nome='$nome'"; //SQL utilizado para a consulta
		$res = $con->query($sql); //Excecuta o comando no banco de dados e armazena a resposta em $res
	...
	if($act == "u"){ //se a acao for atualizar dados
		$sql = "update produtos set imagem='$imagem' where nome='$nome'"; //SQL para a consulta
		$sql2 = "update produtos set idade='$idade' where nome='$nome'"; //SQL para a consulta
	...
	if($act == "d"){ //se a acao for deletar dados
		$sql = "delete from produtos where nome='$nome'"; //SQL para a consulta
		$res = $con->query($sql); //executa a consulta
